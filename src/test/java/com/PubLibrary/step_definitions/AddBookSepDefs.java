...
import com.PubLibrary.pages.AddBookPage;
...
public class AddBookSepDefs {
    DashboardPage dashboardPage = new DashboardPage();
    //BooksPage booksPage = new BooksPage();
    AddBookPage addBookPage = new AddBookPage();
    LibrarianBooksPage booksPage =new LibrarianBooksPage();

    @When("the user navigate to Books module")
    public void the_user_navigate_to_Books_module() {
        BrowserUtils.waitForClickablility(dashboardPage.librarianBooksButton,5);
        dashboardPage.librarianBooksButton.click();
    }

    @When("the user click on Add Book button")
    public void the_user_click_on_Add_Book_button() {
        BrowserUtils.waitForClickablility(booksPage.addBooks,5);
        booksPage.addBooks.click();
    }
    ...
    public void add_Book_button_should_NOT_be_seen_on_the_page() {
        Assert.assertFalse(booksPage.addBooks.isDisplayed());
    }
